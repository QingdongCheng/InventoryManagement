...
var getGroceries = function() {
    ...
    $.ajax({
        "method": "GET",
        "crossDomain": true,
        "url": "http://localhost:8081/api/groceries/all/",
        "success": getAllRes,
        "error": function(err) {
            error_bar.text(err);
        }
...
var getGroceryType = function() {
    ...
    $.ajax({
        ...
        "crossDomain": true,
        "url": "http://localhost:8081/api/groceries/" + type, //Choose your URL path
        "success": getType,
        "error": function(err) {
            error_bar.text(err);
        }
    ...
    if (name === '' || price === '' || type.startsWith("Choose a type:")) {
        ...
        if ($("#item9").text() === "") {
            $.ajax({
                ...
                "crossDomain": true,
                "url": "http://localhost:8081/api/groceries/", //Choose your URL path
                "data": {
                  "name": name,
                  "price": price,
                  "type":type
